...
class Accommodation(db.Model):
    ...
    acc_state_id = db.Column(db.Integer,db.ForeignKey('state.state_id'))  # 房源状态
    acc_address = db.Column(db.String(255))  # 房源详细地址(文字描述)
    acc_city_id = db.Column(db.Integer, db.ForeignKey('city.city_id'))  # 房源城市（下拉列表）
    acc_area_id = db.Column(db.Integer, db.ForeignKey('area.area_id'))  # 房源区域（下拉列表）
    acc_street_id = db.Column(db.Integer, db.ForeignKey('street.street_id'))  # 房源街道（下拉列表）
    ...
    acc_state = db.Relationship('State',backref='state_acclist')
    acc_city = db.Relationship('City', backref='city_acclist')
    acc_area = db.Relationship('Area', backref='area_acclist')
    acc_street = db.Relationship('Street', backref='street_acclist')
    acc_user = db.Relationship('User', backref='user_acclist')
    acc_type = db.Relationship('AccommodationType',backref = 'acctype_acclist')
    
    
    def __init__(self,acc_capacity, acc_price, acc_state_id,acc_address,acc_city_id,
                 acc_area_id,acc_street,acc_description, acc_user_id, acc_type_id):
        self.acc_capacity = acc_capacity
        self.acc_price = acc_price
        self.acc_state_id = acc_state_id
        self.acc_address = acc_address
        self.acc_city_id = acc_city_id
        self.acc_area_id = acc_area_id
        self.acc_street_id = acc_street
    ...
    def to_json(self):
        return jsonify({
                       ...
                       'acc_state_id': self.acc_state_id,
                       'acc_address' : self.acc_address,
                       'acc_city_id' : self.acc_city_id,
                       'acc_area_id' : self.acc_area_id,
                       'acc_street_id' : self.acc_street_id,
...
class Area(db.Model):
    __tablename__='area'
    area_id = db.Column(db.Integer, autoincrement=True, nullable=False, unique=True, primary_key=True)
    area_name = db.Column(db.String(20), nullable=False)
    area_info = db.Column(db.Text)
    def to_json(self):
        return jsonify({
                       'area_name': self.city_name,
                       'area_info': self.info
                       })
#街道
...
class Contract(db.Model):
    ...
    def __init__(self, con_res_id, con_tenant_id, con_lessor_id, con_tenant_option, con_lessor_option, con_state_id,
                 con_disclaimer_id):
